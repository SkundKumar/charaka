...
import { Calendar, Clock, User, Plus, Filter, Search, X, Edit, Trash2, CheckCircle } from 'lucide-react';
...
const AppointmentsPage: React.FC = () => {
  const [doctors, setDoctors] = useState<Doctor[]>([
    { id: 'DOC001', name: 'Dr. Sarah Johnson', specialty: 'Cardiology', availableDays: ['Monday', 'Tuesday', 'Wednesday'], color: '#4F46E5' },
    { id: 'DOC002', name: 'Dr. Michael Chen', specialty: 'Pediatrics', availableDays: ['Monday', 'Thursday', 'Friday'], color: '#10B981' },
    { id: 'DOC003', name: 'Dr. David Rodriguez', specialty: 'Neurology', availableDays: ['Tuesday', 'Wednesday', 'Friday'], color: '#F59E0B' },
    { id: 'DOC004', name: 'Dr. Emily Wilson', specialty: 'Dermatology', availableDays: ['Monday', 'Tuesday', 'Thursday'], color: '#EC4899' },
    { id: 'DOC005', name: 'Dr. Rajesh Patel', specialty: 'Orthopedics', availableDays: ['Wednesday', 'Thursday', 'Friday'], color: '#8B5CF6' },
  ]);
  ...
  const [appointments, setAppointments] = useState<Appointment[]>([
    { id: 'APT001', patientId: 'PAT001', doctorId: 'DOC001', date: '2025-04-06', time: '09:00', duration: 30, type: 'Checkup', status: 'Confirmed', notes: 'Regular blood pressure monitoring' },
    { id: 'APT002', patientId: 'PAT002', doctorId: 'DOC001', date: '2025-04-06', time: '10:00', duration: 45, type: 'Consultation', status: 'Confirmed', notes: 'Diabetes management review' },
    { id: 'APT003', patientId: 'PAT003', doctorId: 'DOC002', date: '2025-04-06', time: '11:00', duration: 30, type: 'Checkup', status: 'Confirmed', notes: 'Asthma control assessment' },
    { id: 'APT004', patientId: 'PAT004', doctorId: 'DOC003', date: '2025-04-07', time: '09:30', duration: 60, type: 'Treatment', status: 'Pending', notes: 'Arthritis pain management' },
    { id: 'APT005', patientId: 'PAT005', doctorId: 'DOC001', date: '2025-04-07', time: '11:30', duration: 30, type: 'Checkup', status: 'Confirmed', notes: 'Blood pressure follow-up' },
    { id: 'APT006', patientId: 'PAT006', doctorId: 'DOC004', date: '2025-04-08', time: '10:00', duration: 45, type: 'Consultation', status: 'Confirmed', notes: 'Migraine treatment plan' },
    { id: 'APT007', patientId: 'PAT007', doctorId: 'DOC002', date: '2025-04-08', time: '14:00', duration: 30, type: 'Checkup', status: 'Confirmed', notes: 'Allergy assessment' },
    { id: 'APT008', patientId: 'PAT008', doctorId: 'DOC005', date: '2025-04-09', time: '09:00', duration: 60, type: 'Treatment', status: 'Confirmed', notes: 'Bone density test' },
    { id: 'APT009', patientId: 'PAT009', doctorId: 'DOC005', date: '2025-04-09', time: '13:00', duration: 45, type: 'Treatment', status: 'Pending', notes: 'Back pain therapy session' },
    { id: 'APT010', patientId: 'PAT010', doctorId: 'DOC004', date: '2025-04-10', time: '11:00', duration: 45, type: 'Consultation', status: 'Confirmed', notes: 'Anxiety management discussion' },
  ]);
  ...
  const handleAddAppointment = () => {
    ...
    const hasConflict = existingAppointments.some(app => {
      const existingStart = parseInt(app.time.replace(':', ''));
      const existingEnd = existingStart + (app.duration / 30) * 50;
      const newStart = parseInt(newAppointment.time.replace(':', ''));
      ...
      doctors.forEach(doctor => {
        if (doctor.availableDays.includes(dayOfWeek)) {
          ...
          const doctorAppointments = appointments.filter(
            app => app.doctorId === doctor.id && app.date === dateStr
          );
  ...
  return (
    <div className="min-h-screen p-6 font-sans bg-gradient-to-br from-blue-50 to-indigo-100">
                ...
                <div className="relative">
                  ...
                  <div className="absolute top-0 bottom-0 left-0 w-16 border-r border-indigo-100">
                    {timeSlots.map((time, index) => (
                      <div key={index} className="h-16 flex items-center justify-center text-sm text-indigo-500 font-medium">
                        {time}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="ml-16 flex">
                    {(selectedDoctor === 'all' ? doctors : doctors.filter(doc => doc.id === selectedDoctor)).map(doctor => (
                      <div key={doctor.id} className="flex-1 min-w-[200px] border-r border-indigo-100 last:border-r-0">
                        <div className="h-12 border-b border-indigo-100 flex items-center justify-center font-bold text-sm" style={{ color: doctor.color }}>
                          {doctor.name}
                        </div>
                        
                        <div className="relative h-[1120px]"> {/* 18 time slots * 64px = 1152px */}
                                ...
                                >
                                  <div className="flex flex-col h-full justify-between overflow-hidden">
                                    <div>
                                      ...
                                      <div className="text-xs truncate">{appointment.time} - {
                                        (() => {
                                    ...
                                    <div className="flex justify-between items-center mt-1">
                                      <span className={`px-2 py-0.5 rounded-full text-xs ${getAppointmentTypeClass(appointment.type)}`}>
                                        {appointment.type}
                                      </span>
                                      <span className={`px-2 py-0.5 rounded-full text-xs ${getStatusBadgeClass(appointment.status)}`}>
                                        {appointment.status}
                                      </span>
                                    </div>
                                  </div>
                ...
                nextAvailableSlots.map((slot, index) => (
                  <li key={index} className="flex items-center gap-3 bg-indigo-50/80 p-3 rounded-lg shadow-sm">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-indigo-600">{slot.dayOfWeek}, {slot.date}</p>
      ...
      {/* Back Button */}
      <button
        className="fixed top-4 left-4 flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition-all shadow-md font-bold"
        onClick={() => window.history.back()} // Navigate to the previous route
      >
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
        </svg>
