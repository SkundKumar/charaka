...
function MedicalRecordsContent() {
  ...
  // Rest of your component logic from the original file
  ...
  const [medicalRecords, setMedicalRecords] = React.useState([
    {
      ...
      content:
        "Patient reports no significant changes in health status. BP: 125/80, Pulse: 72, regular. Weight: 75kg. Heart sounds normal. EKG shows normal sinus rhythm. Recommended continued exercise regimen and follow-up in 6 months.",
    ...
    },
    {
      ...
      content:
        "HbA1c: 7.2% (improved from 7.8%). BP: 130/85. Patient reports good compliance with diet and exercise. Experiencing occasional mild hypoglycemia in mornings. Metformin dosage adjusted to 500mg BID. Encouraged continued lifestyle modifications.",
    ...
    },
    {
      ...
      content:
        "Patient reports decreased frequency of asthma attacks since starting new inhaler. Lung function tests show improvement. PFT: FEV1 85% of predicted (up from 75%). Continued current medication regimen. Advised on seasonal allergen avoidance strategies.",
    ...
    },
    {
      ...
      content:
        "Patient presents with 3-month history of recurring headaches, 2-3 times weekly. Pain described as throbbing, primarily right-sided. No aura or visual disturbances. Neurological exam normal. MRI scheduled. Provisional diagnosis: Migraine without aura. Started on Sumatriptan as needed.",
    ...
    },
    {
      ...
      content:
        "BP readings from home monitoring: averaging 135/85. Current regimen: Lisinopril 10mg daily. Patient reports occasional dizziness upon standing. Recommended sodium restriction, continued exercise. Adjusted medication to Lisinopril 5mg daily. Follow-up in 1 month.",
    ...
    },
    {
      ...
      content:
        "Patient presents with recurring urticarial rash on forearms and neck. Associated with outdoor activity. Skin prick test positive for multiple pollen allergens. Diagnosis: Contact dermatitis with allergic component. Prescribed topical hydrocortisone 1% and oral antihistamine. Referred to allergist for comprehensive testing.",
    ...
    },
    {
      ...
      content:
        "Patient underwent comprehensive allergy panel. Positive reactions to: dust mites, cat dander, birch pollen. Negative for food allergens. Prescribed Fluticasone nasal spray and Cetirizine daily during peak seasons. Provided environmental modification recommendations and allergen avoidance strategies.",
    ...
    },
    {
      ...
      content:
        "Patient reports moderate improvement with current physical therapy regimen. Pain level decreased from 7/10 to 4/10. MRI shows L4-L5 disc bulge without nerve impingement. Continued current PT plan with focus on core strengthening. Maintained current pain management: Naproxen 500mg BID PRN. Discussed ergonomic modifications for workplace.",
    ...
    },
    {
      ...
      content:
        "Patient reports improved anxiety symptoms with current SSRI regimen. Sleep quality improved, reduced frequency of panic attacks (1-2/month vs. 2-3/week previously). Continued Sertraline 50mg daily. Encouraged to maintain CBT sessions. Discussed stress management techniques. Follow-up in 3 months or sooner if symptoms worsen.",
  ...
  const filterRecords = () => {
    ...
    filtered = filtered.sort((a, b) => {
      if (sortField === "date") {
        ...
        return sortDirection === "asc" ? patientA.localeCompare(patientB) : patientB.localeCompare(patientA)
  ...
  // Use the searchParams if needed
  const patientIdFromUrl = searchParams.get("patientId")

  // If there's a patientId in the URL, filter by that patient
  React.useEffect(() => {
    if (patientIdFromUrl) {
                        ...
                        return (
                          <tr key={record.id} className="border-b hover:bg-indigo-50/50 transition-colors">
                            ...
                            <td className="px-4 py-2">
                              <span
                                className={`px-2 py-1 rounded-full text-xs font-bold ${getCategoryBadgeClass(record.category)}`}
                              >
                                {record.category}
                              </span>
                            </td>
                            <td className="px-4 py-2">
                              <span
                                className={`px-2 py-1 rounded-full text-xs font-bold ${getStatusBadgeClass(record.status)}`}
                              >
                                {record.status}
                              </span>
                            </td>
